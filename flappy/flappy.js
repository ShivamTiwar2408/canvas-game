...
function collision_detection (large , small){  
        var collision = false;
        var corners = [];
        corners.push({ x :  small.posX , y : small.posY});
        corners.push({ x :  small.posX + small.dimension , y : small.posY});
        corners.push({ x :  small.posX , y : small.posY + small.dimension});
        corners.push({ x :  small.posX + small.dimension , y : small.posY + small.dimension});
        corners.forEach(c => {
            let cnr_x = c.x;
            let cnr_y =  c.y;
            if (
                cnr_x >= large.posX 
            && cnr_x <= (large.posX + large.dimension)
            && !(
                 cnr_y > large.posY
                 && cnr_y < large.posY + large.opening
                )
            ) collision  = true;
        });  
        return collision;
